...
fn main() -> anyhow::Result<()> {
    ...
    for pn in 1..n {
        ...
        let plain = if let Some(bv) = recover_bv_qb_data(pn, raw) {
            ...
            if candidate[0] == 0 { candidate } else { model.deobfuscate_with_store(raw, pn, &store) }
        };
        let body = &plain[..0xFF0];
        let mut pos = 0usize;
        while pos + 19 <= body.len() {
            if body[pos] == 0x0E && body[pos + 1] == 0x00 && body[pos + 2] == 0x10
                && is_base62(&body[pos + 3..pos + 19])
            {
                let id = std::str::from_utf8(&body[pos + 3..pos + 19]).unwrap().to_owned();
        ...
        if let Some(_d) = &sp.directory {
            pages_with_dir += 1;
            let source = attr.attribute(pn).map(|e| e.name.clone()).unwrap_or_else(|| "<unattributed>".to_string());
            for (_, row) in sp.row_bytes() {
                let scan_end = row.len();
                let mut found = None;
                for i in 0..scan_end.saturating_sub(19) {
                    if row[i] == 0x0E && row[i + 1] == 0x00 && row[i + 2] == 0x10
                        && is_base62(&row[i + 3..i + 19])
                    {
                        let id = std::str::from_utf8(&row[i + 3..i + 19]).unwrap().to_owned();
                        slotted_ids.insert(id.clone());
                        if i < 16 { slotted_ids_first16.insert(id.clone()); }
                        if found.is_none() { found = Some(id); }
                        break;
                    }
                }
                if let Some(id) = found {
                    slotted_per_table.entry(source.clone()).or_default().insert(id);
                }
            }
        }
    }
    ...
    println!("Slotted-scan distinct QB-IDs (anchor anywhere in row): {}", slotted_ids.len());
    println!("Slotted-scan distinct QB-IDs (anchor in first 16 bytes of row): {}", slotted_ids_first16.len());
